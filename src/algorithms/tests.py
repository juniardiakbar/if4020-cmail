...
def test_permutation():
    ...
    """.replace(' ', '').replace('\n', '').lower()
    assert KeccakState.bytes_to_string(s.get_bytes()).encode().hex() == state_after_rounds_on_zero
    ...
    """.replace(' ', '').replace('\n', '').lower()
    assert KeccakState.bytes_to_string(s.get_bytes()).encode('hex') == state_after_rounds_on_previous
...
def test_sponge():
    ...
    """.lower().strip()
    assert KeccakState.bytes_to_string(s.squeeze(512)).encode('hex') == squeezed
